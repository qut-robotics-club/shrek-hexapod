...
// IK constants
const float ARM_SIDE_OFFSET = 0.067 * 100;
const float ARM_FORWARD_OFFSET = 0;
const float SENSOR_HEIGHT = 0.065 * 100;
const float SENSOR_REDUCTION = 0.010 * 100;
const float WRIST_REDUCTION = 0.010 * 100;
const float L1 = 0.0275 * 100;
const float L2 = 0.050 * 100;
...
// move_WalkForward()
void WalkBackwards(float fraction){
  
  int startTime = hexy.millis_new();
  int currentTime = 0;
  int last_update = 0;
  uint16_t maxTime = pgm_read_word_near(move_WalkForward_time + MOVE_WALKFORWARD_SIZE - 1);
  for(int i=MOVE_WALKFORWARD_SIZE-1; i >= MOVE_WALKFORWARD_SIZE*(1-fraction) ; i--){
    delayMicroseconds(10);
    currentTime = hexy.millis_new() - startTime;
    uint16_t move_time = pgm_read_word_near(move_WalkForward_time + i);
    while(maxTime - currentTime > move_time ){
      delayMicroseconds(10);
      currentTime = hexy.millis_new() - startTime;
    }
    uint8_t servo_time = pgm_read_byte_near(move_WalkForward_servo + i);
...
void HighFive(float range, float heading)
{
  ...
  float theta2 = -atand(SENSOR_HEIGHT/abs(r-L1));
  ...
  if (heading <= 0)
  {
    ...
    // move leg to the side to avoid prematurely hitting the hand.
    if (heading >= -45)
    {
      pwmVal = (uint8_t)map(-45,lookRightAngle,lookLeftAngle,lookLeftPos,lookRightPos);
      hexy.changeServo(rightHipServo,pwmVal*10);
      
    }
    else
    {
      pwmVal = (uint8_t)map(70,lookRightAngle,lookLeftAngle,lookLeftPos,lookRightPos);
      hexy.changeServo(rightHipServo,pwmVal*10);
      
    }
    hexy.delay_ms(250);
    
    // position leg for Hi5
...
float isBufferActive(float bufferArray[], float maximum, float minimum, int arraySize)
{
  float average = 0;
...
int canHighFive(float range,float heading)
{
  Serial.println("Starting hi5 test.");
...
void loop() {
  ...
  if (bufferVal != -1)
  {
    ...
    Serial.println("Call canhi5"); 
    int hiFiveStatus = canHighFive(bufferVal,desiredHead);
    
    if (hiFiveStatus == -1)
    {
      Serial.println("backwards");
      WalkBackwards(0.5);
      FillArray(ultrasonicRanges,0.0,BUFFER_SIZE);
    }
    else if (hiFiveStatus == 0)
    {
      HighFive(bufferVal,desiredHead);
      hexy.delay_ms(1000);
      FillArray(ultrasonicRanges,0.0,BUFFER_SIZE);
    ...
    else if (abs(desiredHead) <= rotateThresh)
    {
      ...
      // Default position at this point is a little low at the front. Will fall forwards when lifting a front leg. Angle back a little
      hexy.changeServo(25,2100);
      hexy.changeServo(6,2100);
    }
    else
